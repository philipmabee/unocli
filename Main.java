	...
	public static void main(String[] args) {
		...
		while (true) {
			...
			switch (userInput[0]) {
				...
				case "play":
				case "p":
					GameLogic.playCard(player, userInput);
					break;
					// // if (UserInput.AssertArgC(userInput, 2) != 0) break;
					// if (UserInput.AssertMinArgC(userInput, 2) != 0) break;
					//
					...
					// 		cardIndex[i - 1] = Integer.parseInt(userInput[i - 1]) - 1; // because printed val starts at 1
					// 	}
					//
					// 	// cardIndex = Integer.parseInt(userInput[1]) - 1; // because printed val starts at 1
					// } catch(NumberFormatException e) {
					// 	UserInput.printTextWithColor("'" + userInput[i] + "' is not a number", UserInput.Color.RED);
					// 	break;
					// }
					//
					// Arrays.sort(cardIndex);
					//
					// for (int j = i - 1; j >= 0; j--) {
					// 	switch (player.playCard(cardIndex[j])) {
					// 		case 1:
					// 			UserInput.printTextWithColor("cannot play " + player.getCard(cardIndex[j]).getCardVal() + "\n", UserInput.Color.RED);
					// 			break;
					// 		case 2:
					// 			UserInput.printTextWithColor("Invalid card number" + "\n", UserInput.Color.RED);
					// 			break;
					// 		default:
					// 			break;
					// 	}
...
class GameLogic {
	...
	public static void playCard(Hand player, String[] userInput) {
